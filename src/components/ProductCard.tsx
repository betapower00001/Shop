    ...
    <>
      ...
      <div className="card h-full overflow-hidden border border-gray-200 rounded-lg shadow transition-transform duration-300 hover:scale-105 hover:shadow-xl relative group">
        ...
        <div className="p-4 flex flex-col h-full">
          ...
          <p className="text-sm text-gray-500 line-clamp-2">{safeText(product.description)}</p>
          <p className="text-primary font-bold mt-2 mb-4">฿{product.price.toLocaleString()}</p>

          <button
            className="mt-auto bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition"
            onClick={() => addToCart(1, product, 1)}
          >
            สั่งซื้อ
          </button>
        </div>
